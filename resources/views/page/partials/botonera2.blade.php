...
{{--                        @dd($data->serie)--}}
                        <li class="list-group-item border-0 px-3">
                            <a href="{{ route('serie.sub',$data->id) }}" data-target="#model_{{$key1}}" data-toggle="collapse" aria-expanded="false" class="d-flex align-items-center p-2 border-bottom  ">
                            ...
                            <ul class="list-unstyled collapse  " id="model_{{$key1}}">
                                @forelse($data->product as $data)
                                    ...
                                    <li class=" ">
                                        <a href="{{ route('productos',$data->id) }}" class="px-3 py-2">
                                            <span onclick="location.href='{{ route('producto',$data->id) }}'">{!! $data->text[App::getLocale()]['title'] !!}</span>
                                        </a>
